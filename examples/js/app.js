    ...
    // RUN TRACKING.JS FACIAL DETECTION ON VIDEO STREAM  ===========================================
     ...
     //  }
  	...
  	(function() {
          ...
          $('.dg.ac').detach();
  	    	$('.demo-container').remove(); //this removes the whole webcam feed div (but it doesn't turn off the camera)
      ...
      window.plot = function(x, y, w, h) {
    	...
    	$("#find-eye-mouth-button").click(function() {
      ...
      $("#find-face-button").click(function() {
        var tracker = new tracking.ObjectTracker('face');
          tracker.setStepSize(1.3);
          ...
          tracker.on('track', function(event) {
            if (event.data.length === 0) {
              console.log('that isnt a face, dude');
              $('#status-not-face').removeClass('status-not-face');
            } 
            else {
              event.data.forEach(function(rect) {
                window.plot(rect.x, rect.y, rect.width, rect.height);
                $('#status-face').removeClass('status-face');
              });
            }
          });
